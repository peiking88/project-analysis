...
description: 'Produces comprehensive codebase documentation (8 docs covering stack, structure, architecture, conventions, integrations, testing, concerns, and workflow blueprints). Use this skill whenever the user wants to understand, map, document, or onboard into a codebase — even if they do not say "map" or "document" explicitly. Trigger for prompts like "give me an overview of this repo", "how is this project organized", "what is the tech stack here", "explain the architecture", "help me get up to speed on this codebase", "I just joined this project", "what conventions does this codebase follow", "document this codebase", or any request for repository-level understanding. Do NOT trigger for single-file edits, one-line bug fixes, or narrow feature work.'
license: MIT
...
1. Exactly these files exist in `docs/codebase/`: `STACK.md`, `STRUCTURE.md`, `ARCHITECTURE.md`, `CONVENTIONS.md`, `INTEGRATIONS.md`, `TESTING.md`, `CONCERNS.md`, `WORKFLOWS.md`.
...
1. Run the scan script from the target project root:
   ...
   **Quick start:** If you have the path inline:
   ```bash
   python3 /absolute/path/to/skills/project-analysis/scripts/scan.py --output docs/codebase/.codebase-scan.txt
   ```
...
8. [WORKFLOWS.md](assets/templates/WORKFLOWS.md) — E2E traces, layer implementation patterns, implementation templates
...
| [`assets/templates/WORKFLOWS.md`](assets/templates/WORKFLOWS.md) | Phase 3 step 8 |

The `references/stack-detection.md` file now includes a **Technology-Specific Implementation Patterns** appendix — load it during Phase 2 workflow investigation to recognize layer conventions in .NET, Spring, React, Node.js, and Python stacks.
